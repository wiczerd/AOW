...
sys.path.insert(0, os.path.abspath('/home/david/Dropbox/Search and Networks In Labor Markets/Computations/Python'))
...
for zi in range(0,zpts):
    r1z[zi] = rhoz1(zi+1,nz)
    r0z[zi] = rhoz0(zi+1,nz)
...
nu0steps = np.append(0.1, np.linspace(.1,1.,nstep-1)**.25*nu0)
...
for homotop_i in range(0,nstep):
    ...
    for Fi in range(0, maxFiter):
            ...
            for wi in range(1,wpts-1):
                ...
                if zRm1 <= (zpts-1):
                    ...
                    # by linear interpolation, find points beyond the threshold
                        ...
                        # IS THIS CORRECT or should it interpolate on zgrid?
                    ...
                    beta_wz = beta_wz[indicz[wi]]
                    hdir_wz = hdir_wz[indicz[wi]]
                    href_wz = href_wz[indicz[wi]]
                    zg_endo = np.append(zgrid[0:zRm1],zindif)
                        
                    lwz_resid = lambda lwzH: lwzH*beta_wz - hdir_wz - np.trapz(lwzH,zg_endo)*href_wz

                    lbarwTemp, infodict, flag, mesg = fsolve(lwz_resid, Omegaz/M, full_output=True)
                    lwz[wi,0:zR1] = lbarwTemp.copy()
                    lwz[wi,zR1+1:zpts] = 0
                    Lw[wi] = np.trapz(lwz[wi,0:zR1])
                ...
                diryield_wz[wi,0:len(hdir_wz)] = hdir_wz.copy()
                refyield_wz[wi,0:len(href_wz)] = href_wz*np.trapz(lwz[wi,0:zR1],zg_endo)
        ...
        # Now solve for F1 at each wi
        F1 = np.zeros(wpts)
        F1w_normresid = np.zeros(wpts)
        hwz0 = np.zeros((wpts,zpts))
